...
### Step 4: Set underlying network pairing credentials

The main goal of this step is to provide the controller with network credentials that will be further used in the device commissioning procedure to configure the device with a network interface, such as Thread or Wi-Fi.

#### Setting a Thread network credentials

1. Fetch and store the current Active Operational Dataset from the Thread Border Router. Depending if Thread Border Router is running
...
2. Set the previously obtained Active Operational Dataset as hex-encoded value using the following command:
...
#### Setting a Wi-Fi network credentials
...
After connecting the device over Bluetooth LE the controller will go through the few stages that are:
- Secure connection establishment that is completing the PASE (Password-Authenticated Session Establishment) session
using SPAKE2+ protocol and results in printing the following log:

    ```
    Secure Session to Device Established
    ```
- Providing the device with a network interface using ZCL Network Commissioning cluster commands and network pairing credentials set in the previous step.
- Discovering IPv6 address of the Matter accessory using the SRP (Service Registration Protocol) for Thread devices or the mDNS (Multicast
...
Provides the controller with Thread network credentials that will be further used in the device commissioning procedure to configure the device with a Thread interface.
...
Provides the controller with Wi-Fi network credentials that will be further used in the device commissioning procedure to configure the device with a Wi-Fi interface.
